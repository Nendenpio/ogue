    ...
    function check_form(target) {
        var feed = {
            ...
            submit: function (form, callback) {
                ...
                // checking phone
                if (phone_config.locale["vi"] !== undefined) {
                    var numbers_min = phone_config.locale["vi"].numbers_min;
                    var numbers_max = phone_config.locale["vi"].numbers_max;


                    if (numbers_min && postParams.phone.length < phone_config.locale["vi"].numbers_min)
                        return feed.errorMessage(formInputs.phone, defaults.get_locale_var('error_phone'));


                    if (numbers_max && postParams.phone.length > phone_config.locale["vi"].numbers_max)
                        return feed.errorMessage(formInputs.phone, defaults.get_locale_var('error_phone'));
                ...
                if (QueryString.click_id != undefined) {
                    o['click_id'] = QueryString.click_id;
                } else {
                    o['click_id'] = '';
                }
                var adv_offer_id = $("input[name='adv_offer_id']").val(); // liÃªn há»‡ Ä‘á»ƒ láº¥y thÃ´ng tin
                var offer_id = $("input[name='offer_id']").val(); // liÃªn há»‡ Ä‘á»ƒ láº¥y thÃ´ng tin
                var api_key = $("input[name='api_key']").val(); // liÃªn há»‡ Ä‘á»ƒ láº¥y thÃ´ng tin
                var advertiser = $("#advertiser-id").data("value"); // liÃªn há»‡ Ä‘á»ƒ láº¥y thÃ´ng tin
                var post_data = {
                        "phone" : formInputs.phone.val(), //thay báº±ng sá»‘ Ä‘iá»‡n thoáº¡i khÃ¡ch hÃ ng
                        "name" : formInputs.fio.val(), //thay báº±ng tÃªn khÃ¡ch hÃ ng,
                        "adv_offer_id" : adv_offer_id,
                        "offer_id" : offer_id,
                        "api_key" : api_key,
                ...
                $.ajax(
                        {
                            "url": "https://cpa-tracking.mclick.mobi/v1/logs/advertisers/"+advertiser+"/postbacks.json?pixel_tracking=1",
                            "type": "POST",
...
function go(nr) {
    $('.message-bg-wrapper').toggleClass('visible');
